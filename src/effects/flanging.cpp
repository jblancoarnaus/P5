...
Flanging::Flanging(const std::string &param)
{
	KeyValue kv(param);
	...
	if (!kv.to_float("A_flang", A_flang))
		A_flang = 0.1; //default value
		
	if (!kv.to_float("g", g))
		g = 0.1; //default value

	if (!kv.to_float("M_o", M_o))
		M_o = 0.1; //default value
...
void Flanging::command(unsigned int cmd)
{
	fase_mod = 0;
	if (cmd == 0)
		buffer.resize(0);
...
void Flanging::operator()(std::vector<float> &x)
{
	...
	for (tot = 0; fase_sen < buffer.size() and tot < x.size(); tot++)
	{
		...
		xout[tot] = x[tot]+ g*(xant + rho * (xpos - xant));

		fase_sen += 1 - M_o*(1 + A_flang * sin(fase_mod));
